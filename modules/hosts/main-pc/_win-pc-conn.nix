{pkgs-current, ...}: {
  ...
  systemd.user.services.kdeconnectd = {
    ...
    serviceConfig = {
      ExecStart = "${pkgs-current.kdePackages.kdeconnect-kde}/bin/kdeconnectd";
      Restart = "on-failure";
      RestartSec = 5;
    };
  };
  ...
  systemd.user.services.kdeconnect-indicator = {
    ...
    serviceConfig = {
      ExecStart = "${pkgs-current.kdePackages.kdeconnect-kde}/bin/kdeconnect-indicator";
      Restart = "on-failure";
      RestartSec = 5;
    };
  };
  ...
  networking = {
    networkmanager.unmanaged = ["interface-name:enp13s0"];
