{...}: {
  flake.nixosModules.reverseProxySiteSearxNG = {
    config,
    lib,
    ...
  }: let
    cfg = config.dotfiles.services.reverseProxy;
    siteCfg = cfg.sites.searxng;
  in {
    options.dotfiles.services.reverseProxy.sites.searxng.enable = lib.mkEnableOption "SearxNG reverse proxy site";
